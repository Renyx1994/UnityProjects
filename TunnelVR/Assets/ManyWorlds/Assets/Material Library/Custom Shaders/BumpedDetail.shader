...
/////// Normals:
                ...
                float3 node_134_nrm_base = UnpackNormal(tex2D(_NormalMain,TRANSFORM_TEX(node_356.rg, _NormalMain))).rgb + float3(0,0,1);
                float3 node_134_nrm_detail = UnpackNormal(tex2D(_NormalDetail,TRANSFORM_TEX(node_356.rg, _NormalDetail))).rgb * float3(-1,-1,1);
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float3 specularColor = ((_SpecularTint.rgb*tex2D(_Specular,TRANSFORM_TEX(node_356.rg, _Specular)).rgb)*_SpecularIntensity);
                ...
                #endif
                finalColor += diffuseLight * (_ColorTint.rgb*saturate((tex2D(_DiffuseMain,TRANSFORM_TEX(node_356.rg, _DiffuseMain)).rgb+tex2D(_DiffuseDetail,TRANSFORM_TEX(node_356.rg, _DiffuseDetail)).rgb-1.0)));
                finalColor += specular;
/// Final Color:
...
/////// Normals:
                ...
                float3 node_134_nrm_base = UnpackNormal(tex2D(_NormalMain,TRANSFORM_TEX(node_357.rg, _NormalMain))).rgb + float3(0,0,1);
                float3 node_134_nrm_detail = UnpackNormal(tex2D(_NormalDetail,TRANSFORM_TEX(node_357.rg, _NormalDetail))).rgb * float3(-1,-1,1);
                float3 node_134_nrm_combined = node_134_nrm_base*dot(node_134_nrm_base, node_134_nrm_detail)/node_134_nrm_base.z - node_134_nrm_detail;
                float3 node_134 = node_134_nrm_combined;
                float3 normalLocal = node_134;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float3 specularColor = ((_SpecularTint.rgb*tex2D(_Specular,TRANSFORM_TEX(node_357.rg, _Specular)).rgb)*_SpecularIntensity);
                float3 specular = attenColor * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                finalColor += diffuseLight * (_ColorTint.rgb*saturate((tex2D(_DiffuseMain,TRANSFORM_TEX(node_357.rg, _DiffuseMain)).rgb+tex2D(_DiffuseDetail,TRANSFORM_TEX(node_357.rg, _DiffuseDetail)).rgb-1.0)));
                finalColor += specular;
/// Final Color:
