...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_NormalMap,TRANSFORM_TEX(node_253.rg, _NormalMap))).rgb;
...
/////// Diffuse:
                ...
                #endif
////// Emissive:
                float4 node_35 = tex2D(_SpecularRGBGlossA,TRANSFORM_TEX(node_253.rg, _SpecularRGBGlossA));
...
////// Specular:
                ...
                finalColor += diffuseLight * (tex2D(_Base,TRANSFORM_TEX(node_253.rg, _Base)).rgb*_ColorTint.rgb);
                finalColor += specular;
                finalColor += emissive;
/// Final Color:
...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_NormalMap,TRANSFORM_TEX(node_254.rg, _NormalMap))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                float3 viewReflectDirection = reflect( -viewDirection, normalDirection );
...
///////// Gloss:
                float4 node_35 = tex2D(_SpecularRGBGlossA,TRANSFORM_TEX(node_254.rg, _SpecularRGBGlossA));
                float gloss = (node_35.a*_GlossLevel);
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                ...
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                finalColor += diffuseLight * (tex2D(_Base,TRANSFORM_TEX(node_254.rg, _Base)).rgb*_ColorTint.rgb);
                finalColor += specular;
/// Final Color:
